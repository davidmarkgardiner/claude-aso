...
import {
  ...
  NamespaceRequest,
} from "../services/namespaceProvisioning";
...
const namespaceRequestSchema = Joi.object({
  ...
  rbacConfig: Joi.object({
    principalId: Joi.string().guid().required().messages({
      "string.guid": "Principal ID must be a valid GUID",
    }),
    principalType: Joi.string().valid("User", "Group").default("User"),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const result =
        await provisioningService.provisionNamespace(namespaceRequest);
      ...
      if (value.rbacConfig) {
        ...
        } catch (rbacError: unknown) {
          logger.error("RBAC provisioning failed", {
            requestId: result.requestId,
            namespaceName: namespaceRequest.namespaceName,
            error:
              rbacError instanceof Error ? rbacError.message : "Unknown error",
          });
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const namespaceDetails =
        await provisioningService.getNamespaceDetails(namespaceName);

      // Check if user has access to this namespace's team
      const namespaceTeam =
        namespaceDetails.namespace.metadata?.labels?.["platform.io/team"];
      ...
      return res.json({
        ...
        data: {
          namespace: {
            ...
            team: namespaceTeam,
            environment:
              namespaceDetails.namespace.metadata?.labels?.[
                "platform.io/environment"
              ],
            creationTimestamp:
              namespaceDetails.namespace.metadata?.creationTimestamp,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const rbacStatus = await rbacService.getRBACStatus(
        namespaceName,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const { namespaceName } = req.params;
    const { principalId, principalType, roleDefinition, clusterName } =
      req.body;
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      await rbacService.removeNamespaceRBAC(
        namespaceName,
        clusterName as string,
      );
