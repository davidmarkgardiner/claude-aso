...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!includeInactive) {
      mockTeamData.teams = mockTeamData.teams.filter(
        (team) => team.activeNamespaces > 0,
      );
    }
        ...
        recommendations: [
          {
            type: "rightsizing",
            potential: "$450",
            description: "Reduce over-allocated CPU/memory",
          },
          {
            type: "scheduling",
            potential: "$380",
            description: "Better workload scheduling",
          },
          {
            type: "storage",
            potential: "$415",
            description: "Cleanup unused storage volumes",
          },
        ],
      },
    };
