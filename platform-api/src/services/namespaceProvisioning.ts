...
export class NamespaceProvisioningService {
  ...
  async provisionNamespace(
    ...
    try {
      ...
      await this.storeRequestMetadata(
        requestId,
        request,
      ...
      logger.error(
        `Failed to provision namespace for request ${requestId}:`,
        error,
      );
      throw error;
    }
  ...
  async getProvisioningStatus(requestId: string): Promise<ProvisioningResult> {
    try {
      ...
      if (requestMetadata.workflowId) {
        const workflowStatus = await this.argoClient.getWorkflowStatus(
      ...
      logger.error(
        `Failed to get provisioning status for request ${requestId}:`,
        error,
      );
      throw error;
    }
  ...
  async cancelProvisioning(
    ...
    try {
      ...
      logger.info(`Namespace provisioning cancelled`, {
        requestId,
        cancelledBy,
      });
    } catch (error) {
      logger.error(
        `Failed to cancel provisioning for request ${requestId}:`,
        error,
      );
      throw error;
    }
  ...
  async getNamespaceDetails(namespaceName: string): Promise<{
    ...
    try {
      ...
      logger.error(
        `Failed to get namespace details for ${namespaceName}:`,
        error,
      );
      throw error;
    }
  ...
  private async validateRequest(request: NamespaceRequest): Promise<void> {
    ...
    if (!namePattern.test(request.namespaceName)) {
      throw new Error(
        "Invalid namespace name format. Must be lowercase alphanumeric with hyphens.",
      );
    }
    ...
    const existingNamespace = await this.k8sClient.getNamespace(
    ...
    if (teamNamespaces.length >= config.platform.maxNamespacesPerTeam) {
      throw new Error(
        `Team ${request.team} has reached namespace quota limit (${config.platform.maxNamespacesPerTeam})`,
      );
    }
    ...
    if (request.environment === "production") {
      ...
      if (request.networkPolicy === "open") {
        throw new Error(
          'Production environments cannot use "open" network policy',
        );
      }
    }
  }

  private generateWorkflowSpec(
    request: NamespaceRequest,
    requestId: string,
  ): any {
      ...
      spec: {
        ...
        arguments: {
          parameters: [
            ...
            {
              name: "max-services",
              value: resourceTierConfig.maxServices.toString(),
            },
                ...
                },
                {
                  ...
                  arguments: {
                    parameters: [
                      ...
                      {
                        name: "storage-quota",
                        value: "{{workflow.parameters.storage-quota}}",
                      },
                      {
                        name: "max-pods",
                      ...
                      {
                        name: "max-services",
                        value: "{{workflow.parameters.max-services}}",
                      },
                    ],
                  },
  ...
  private mapWorkflowStatusToRequestStatus(
  ...
  private async getNetworkPolicies(
    _namespace: string,
