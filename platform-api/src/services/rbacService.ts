...
export const RBAC_ERROR_CODES = {
  PRINCIPAL_NOT_FOUND: {
    status: 400,
    message: "User or group not found in Azure AD",
    retryable: false,
  },
  ASO_TIMEOUT: {
    status: 408,
    message: "Role assignment timeout - check status later",
    retryable: true,
  },
  PERMISSION_DENIED: {
    status: 403,
  ...
  CLUSTER_UNAVAILABLE: {
    status: 503,
    message: "Target cluster temporarily unavailable",
    retryable: true,
  },
  APPROVAL_REQUIRED: {
    status: 202,
...
export class RBACService {
  ...
  async provisionNamespaceRBAC(
    ...
    try {
      logger.info("Starting RBAC provisioning", {
        ...
        options: {
          ...options,
          principalId: this.maskPrincipalId(options.principalId),
        },
      });
      ...
      const clusterConfig = await this.getClusterConfigWithValidation(
        options.clusterName,
        environment,
      );
      ...
      if (!options.skipValidation) {
        const validationResult = await this.validatePrincipalWithRetry(
          options.principalId,
        );
        ...
        auditEvent.principalType =
          validationResult.principalType || auditEvent.principalType;
      }
      ...
      const result: RBACProvisioningResult = {
        ...
        roleAssignmentIds: asoManifests.map(
  ...
  async getRBACStatus(
    ...
    try {
      ...
      logger.error("Failed to get RBAC status", {
        namespaceName,
  ...
  async removeNamespaceRBAC(
    ...
    try {
      ...
      logger.info("RBAC resources removed successfully", {
        namespaceName,
  ...
  private getClusterConfig(
    ...
    if (environment) {
      const clusters =
        this.clusterConfigService.getClustersByEnvironment(environment);
  ...
  private createRoleAssignmentRequest(
    ...
    const namespaceScope =
      this.clusterConfigService.generateNamespaceScopeArmId(
        clusterConfig,
        namespaceName,
      );
      ...
      const manifest: AzureServiceOperatorRoleAssignment = {
        ...
        metadata: {
          ...
          annotations: {
            ...
            "platform.io/role-definition":
              assignment.roleDefinitionId.split("/").pop() || "unknown",
          },
        },
        spec: {
          owner: {
  ...
  private async applyASOManifests(
    manifests: AzureServiceOperatorRoleAssignment[],
  ): Promise<void> {
  ...
  private async getClusterConfigWithValidation(
    clusterName?: string,
    environment?: string,
  ...
  private async checkResourceQuotas(
    ...
    try {
      ...
      if (
        ...
        existing.items.length >= maxAssignments
      ) {
        throw new RBACError(
      ...
      logger.warn("Resource quota check failed", {
        namespaceName,
        teamName,
        error:
  ...
  private async validatePrincipalWithRetry(
    principalId: string,
    maxRetries: number = 3,
  ): Promise<any> {
  ...
  private async applyASOManifestsWithRetry(
    manifests: AzureServiceOperatorRoleAssignment[],
    ...
    try {
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("ASO manifest application timeout")),
            timeout,
          ),
        ),
      ]);
  ...
  private generateNamespaceScopeArmId(
    clusterConfig: ClusterConfiguration,
    namespaceName: string,
  ): string {
  ...
  async validatePlatformApiPermissions(): Promise<boolean> {
    try {
      ...
      logger.error("Platform API permissions validation failed", {
        error: error instanceof Error ? error.message : "Unknown error",
      });
      return false;
    }
  ...
  async createNamespaceWithEnhancedRBAC(request: {
    name: string;
    teamName: string;
    environment: string;
    teamPrincipalId: string;
    ...
    try {
      ...
      if (request.resourceTier) {
        await this.applyResourceQuotaForTier(
          request.name,
          request.resourceTier,
        );
      }
      ...
      logger.info(
        "Enhanced namespace creation with RBAC completed successfully",
        {
          namespaceName: request.name,
          teamName: request.teamName,
          duration: Date.now() - startTime,
  ...
  private async applyResourceQuotaForTier(
    namespaceName: string,
    tier: string,
  ): Promise<void> {
    const quotaSpecs = {
  ...
  private async applyDefaultNetworkPolicies(
    namespaceName: string,
  ): Promise<void> {
