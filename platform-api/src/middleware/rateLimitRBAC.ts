...
export const rbacRateLimit = rateLimit({
  ...
  handler: async (req: Request, res: Response) => {
    ...
    await auditService.logRBACEvent({
      ...
      correlationId: req.headers['x-correlation-id'] as string || 'rate-limited',
...
export const adminRoleRateLimit = rateLimit({
  ...
  handler: async (req: Request, res: Response) => {
    ...
    await auditService.logRBACEvent({
      ...
      sourceIP: req.ip || 'unknown',
      userAgent: req.get('user-agent') || 'unknown',
      correlationId: req.headers['x-correlation-id'] as string || 'admin-rate-limited',
      timestamp: new Date().toISOString(),
      success: false,
      error: 'Admin role assignment rate limit exceeded - potential security concern'
    });

    res.status(429).json({
      error: 'ADMIN_RATE_LIMIT_EXCEEDED',
      message: 'Too many admin role assignment requests. Admin role assignments are limited for security reasons.',
      retryAfter: 3600, // 1 hour in seconds
      timestamp: new Date().toISOString(),
      recommendation: 'Consider using lower-privileged roles or contact your administrator'
    });
  }
});
