    ...
    try {
      ...
      if (error instanceof CircuitBreakerError) {
        ...
        return {
          ...
          principal: undefined,
          errors: [
            `Azure AD service temporarily unavailable: ${error.message}`,
          ],
        };
      }
      ...
      return {
        valid: false,
        errors: [
          `Failed to validate user '${userPrincipalName}': ${error instanceof Error ? error.message : "Unknown error"}`,
        ],
      };
    }
  }

  async validateGroupPrincipal(
    ...
    try {
      ...
      return {
        valid: false,
        errors: [
          `Failed to validate group '${groupObjectId}': ${error instanceof Error ? error.message : "Unknown error"}`,
        ],
      };
    }
  }
  ...
  async validatePrincipalById(objectId: string): Promise<RBACValidationResult> {
    try {
      ...
      return {
        valid: false,
        errors: [
          `Failed to validate principal '${objectId}': ${error instanceof Error ? error.message : "Unknown error"}`,
        ],
      };
    }
  }
...
export const validateAzureADPrincipal = (
  principalField: string = "principalId",
) => {
  return async (
    req: Request,
    res: Response,
    ...
    try {
      ...
      const validationResult =
        await validationService.validatePrincipalById(principalId);
