...
function mapGroupsToRoles(groups: string[]): string[] {
  const roleMapping: Record<string, string[]> = {
    ...
    'Frontend-Team-Developers': ['team:frontend:developer', 'namespace:developer'],
    'Backend-Team-Admins': ['team:backend:admin', 'namespace:admin'],
    'Backend-Team-Developers': ['team:backend:developer', 'namespace:developer'],
...
export const requireTeamAccess = (req: Request, res: Response, next: NextFunction): void => {
  ...
  const hasTeamAccess = req.user.roles.some(role => 
    role.startsWith(`team:${requestedTeam}:`) || role === `team:${requestedTeam}`
  );
...
export const generatePlatformToken = (user: Partial<AuthenticatedUser>): string => {
  const payload = {
    sub: user.id,
    email: user.email,
    name: user.name,
