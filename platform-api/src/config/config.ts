...
const configSchema = z.object({
  ...
  corsOrigins: z.string().transform(str => str.split(',').map(s => s.trim())),
...
const rawConfig = {
  ...
  corsOrigins: process.env.CORS_ORIGINS || 'http://localhost:3000,http://localhost:7007',
  ...
  platform: {
    ...
    maxNamespacesPerTeam: parseInt(process.env.PLATFORM_MAX_NAMESPACES_PER_TEAM || '10', 10),
    allowedFeatures: (process.env.PLATFORM_ALLOWED_FEATURES || 
    ...
    auditLogRetentionDays: parseInt(process.env.PLATFORM_AUDIT_RETENTION_DAYS || '90', 10)
  },
