...
export class ClusterConfigurationService {
    ...
    const defaultClusters: ClusterConfiguration[] = [
      {
        ...
        armId: process.env.DEV_AKS_ARM_ID || '/subscriptions/133d5755-4074-4d6e-ad38-eb2a6ad12903/resourceGroups/aks-cluster/providers/Microsoft.ContainerService/managedClusters/dev-cluster',
        resourceGroup: process.env.DEV_AKS_RG || 'aks-cluster',
        subscriptionId: process.env.AZURE_SUBSCRIPTION_ID || '133d5755-4074-4d6e-ad38-eb2a6ad12903',
        region: 'uksouth',
      ...
      },
      {
        ...
        armId: process.env.STAGING_AKS_ARM_ID || '/subscriptions/133d5755-4074-4d6e-ad38-eb2a6ad12903/resourceGroups/aks-cluster/providers/Microsoft.ContainerService/managedClusters/staging-cluster',
      ...
      },
      {
        ...
        armId: process.env.PROD_AKS_ARM_ID || '/subscriptions/133d5755-4074-4d6e-ad38-eb2a6ad12903/resourceGroups/aks-cluster/providers/Microsoft.ContainerService/managedClusters/uk8s-tsshared-weu-gt025-int-prod',
  ...
  getClustersByEnvironment(environment: string): ClusterConfiguration[] {
    return Array.from(this.clusters.values())
      .filter(cluster => cluster.environment === environment);
  }

  getDefaultCluster(): ClusterConfiguration | undefined {
    return Array.from(this.clusters.values())
      .find(cluster => cluster.isDefault);
  }
  ...
  generateNamespaceScopeArmId(clusterConfig: ClusterConfiguration, namespaceName: string): string {
  ...
  validateClusterConfiguration(cluster: ClusterConfiguration): string[] {
    ...
    if (!['development', 'staging', 'production'].includes(cluster.environment)) {
  ...
  private isValidArmId(armId: string): boolean {
    const armIdPattern = /^\/subscriptions\/[a-f0-9-]+\/resourceGroups\/[^/]+\/providers\/Microsoft\.ContainerService\/managedClusters\/[^/]+$/i;
