    ...
    return new Promise(async (resolve, reject) => {
      if (this.state === CircuitState.OPEN) {
        if (Date.now() < this.nextAttempt) {
          ...
          return reject(new CircuitBreakerError(
      ...
      const timeoutId = setTimeout(() => {
        this.onFailure(new Error(`Request timeout after ${this.options.timeout}ms`));
  ...
  private onSuccess(): void {
    ...
    if (this.requestCount % 50 === 0) {
      logger.debug('Circuit breaker health check', {
        ...
        failures: this.failures,
        successRate: ((this.requestCount - this.failures) / this.requestCount * 100).toFixed(2) + '%'
      });
    }
  }
  ...
  getMetrics() {
    ...
    return {
      ...
      successRate: this.requestCount > 0 ? 
        ((this.requestCount - this.failures) / this.requestCount * 100).toFixed(2) + '%' : 'N/A',
      isHealthy: this.state === CircuitState.CLOSED,
      nextAttemptIn: this.state === CircuitState.OPEN ? 
        Math.max(0, Math.ceil((this.nextAttempt - now) / 1000)) : 0,
