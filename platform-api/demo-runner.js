...
const mockTemplates = [
  {
    ...
    description: 'A production-ready REST API microservice with database integration, monitoring, and security features',
    ...
    parameters: [
      ...
      { name: 'replicas', type: 'number', required: false, description: 'Number of replicas to deploy', defaultValue: 2 }
    ],
    examples: [
      { name: 'Basic Payment API', description: 'Simple payment processing API with PostgreSQL' },
      { name: 'High-Performance User API', description: 'User management API with MongoDB and high availability' }
    ]
  },
  {
    id: 'static-website',
    ...
    parameters: [
      ...
      { name: 'domain', type: 'string', required: true, description: 'Domain name for the website' }
    ]
  }
];
...
mockNamespaces.forEach((ns, index) => {
  ...
  console.log(`      Workloads: ${ns.resources.pods} pods, ${ns.resources.services} services`);
...
const avgCpuUsage = Math.round(mockNamespaces.reduce((sum, ns) => sum + ns.quota.cpu.percentage, 0) / mockNamespaces.length);
const avgMemoryUsage = Math.round(mockNamespaces.reduce((sum, ns) => sum + ns.quota.memory.percentage, 0) / mockNamespaces.length);
