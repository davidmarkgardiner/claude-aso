...
const mockTemplates = require('./tests/fixtures/templates.js').mockTemplates || [
  {
    id: 'microservice-api',
    ...
    parameters: [
      ...
      { name: 'databaseType', type: 'select', required: true, description: 'Database type', options: ['postgresql', 'mysql'] },
      { name: 'replicas', type: 'number', required: false, defaultValue: 2, description: 'Number of replicas' },
      { name: 'resourceTier', type: 'select', required: false, defaultValue: 'small', options: ['micro', 'small', 'medium', 'large'] }
    ],
    examples: [
      { name: 'Payment API', description: 'Payment processing service', parameters: { serviceName: 'payment-api', databaseType: 'postgresql', replicas: 2 } },
      { name: 'User API', description: 'User management service', parameters: { serviceName: 'user-api', databaseType: 'mysql', replicas: 3 } }
    ]
  },
  {
    id: 'static-website',
    ...
    parameters: [
      ...
      { name: 'framework', type: 'select', required: true, options: ['react', 'nextjs', 'vue'] },
  ...
  },
  {
    ...
    parameters: [
      ...
      { name: 'queueType', type: 'select', required: true, options: ['redis', 'rabbitmq'] },
  ...
  },
  {
    ...
    parameters: [
      { name: 'engine', type: 'select', required: true, options: ['postgresql', 'mysql'] },
...
const mockNamespaces = [
  {
    ...
    resources: { pods: 12, services: 4, deployments: 6, configMaps: 8, secrets: 3 },
    quota: {
  ...
  },
  {
    ...
    resources: { pods: 18, services: 6, deployments: 8, configMaps: 12, secrets: 5 },
    quota: {
  ...
  },
  {
    ...
    resources: { pods: 45, services: 15, deployments: 20, configMaps: 25, secrets: 12 },
    quota: {
  ...
  },
  {
    ...
    resources: { pods: 8, services: 3, deployments: 4, configMaps: 6, secrets: 2 },
    quota: {
  ...
  },
  {
    ...
    resources: { pods: 0, services: 0, deployments: 0, configMaps: 0, secrets: 0 },
    quota: {
...
const mockProvisioningRequests = [
  {
    ...
    workflowStatus: { phase: 'Succeeded', message: 'Namespace provisioned successfully' }
  },
  {
    requestId: 'req-456',
...
const mockDeployments = [
  {
    ...
    createdAt: '2023-01-01T12:00:00Z',
    completedAt: '2023-01-01T12:15:00Z',
    parameters: { databaseType: 'postgresql', replicas: 2, resourceTier: 'small' }
  },
  {
...
console.log(`📊 Provisioning Requests (${mockProvisioningRequests.length} total):\n`);
...
mockProvisioningRequests.forEach((request, index) => {
  ...
  console.log(`      Team: ${request.team} | Environment: ${request.environment}`);
  ...
  if (request.completedAt) {
    console.log(`      Completed: ${new Date(request.completedAt).toLocaleDateString()}`);
  }
  ...
  if (request.workflowStatus) {
    console.log(`      Workflow: ${request.workflowStatus.phase} - ${request.workflowStatus.message}`);
  }
  console.log('');
});
...
mockNamespaces.forEach((ns, index) => {
  ...
  console.log(`      Resource Tier: ${ns.resourceTier} | Policy: ${ns.networkPolicy}`);
  ...
  if (ns.resources && ns.status === 'active') {
    console.log(`      Workloads: ${ns.resources.pods} pods, ${ns.resources.services} services, ${ns.resources.deployments} deployments`);
  }
...
mockDeployments.forEach((deployment, index) => {
  ...
  console.log(`      Team: ${deployment.team} | Namespace: ${deployment.namespace}`);
  console.log(`      Environment: ${deployment.environment}`);
  console.log(`      Status: ${deployment.status} (${deployment.progress}% complete)`);
  console.log(`      Created: ${new Date(deployment.createdAt).toLocaleDateString()}`);
  
  if (deployment.completedAt) {
    console.log(`      Completed: ${new Date(deployment.completedAt).toLocaleDateString()}`);
  }
...
users.forEach((user, index) => {
  const roleIcon = user.roles.includes('platform:admin') ? '👑' : 
                  user.roles.includes('namespace:admin') ? '🛡️' : '👨‍💻';
...
const activeNamespaces = mockNamespaces.filter(ns => ns.status === 'active').length;
const activeDeployments = mockDeployments.filter(d => d.status === 'completed' || d.status === 'running').length;
const totalTeams = [...new Set(mockNamespaces.map(ns => ns.team))].length;

// Resource utilization (average across active namespaces)
const activeNs = mockNamespaces.filter(ns => ns.status === 'active' && ns.quota);
const avgCpuUsage = activeNs.length > 0 ? Math.round(activeNs.reduce((sum, ns) => sum + ns.quota.cpu.percentage, 0) / activeNs.length) : 0;
const avgMemoryUsage = activeNs.length > 0 ? Math.round(activeNs.reduce((sum, ns) => sum + ns.quota.memory.percentage, 0) / activeNs.length) : 0;
...
console.log(`   Resource Utilization: ${avgCpuUsage}% CPU, ${avgMemoryUsage}% Memory`);
console.log('');
...
Object.entries(tierCounts).forEach(([tier, count]) => {
  const estimatedCost = { micro: 50, small: 100, medium: 200, large: 400 }[tier] || 0;
  console.log(`   ${tier}: ${count} namespaces (~$${estimatedCost * count}/month)`);
});

const totalEstimatedCost = Object.entries(tierCounts).reduce((sum, [tier, count]) => {
...
// Success rates
const completedRequests = mockProvisioningRequests.filter(r => r.status === 'completed').length;
const successRate = mockProvisioningRequests.length > 0 ? Math.round((completedRequests / mockProvisioningRequests.length) * 100) : 0;

const completedDeployments = mockDeployments.filter(d => d.status === 'completed').length;
const deploymentSuccessRate = mockDeployments.length > 0 ? Math.round((completedDeployments / mockDeployments.length) * 100) : 0;
