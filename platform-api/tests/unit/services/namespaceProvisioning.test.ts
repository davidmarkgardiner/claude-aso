...
describe("NamespaceProvisioningService", () => {
  ...
  beforeEach(() => {
    mockKubernetesClient =
      new KubernetesClient() as jest.Mocked<KubernetesClient>;
    mockArgoClient =
      new ArgoWorkflowsClient() as jest.Mocked<ArgoWorkflowsClient>;
  ...
  describe("provisionNamespace", () => {
    ...
    it("should successfully provision a namespace", async () => {
      ...
      expect(mockKubernetesClient.namespaceExists).toHaveBeenCalledWith(
        "test-app-dev",
      );
    ...
    it("should handle workflow submission failures", async () => {
      // Arrange
      mockKubernetesClient.namespaceExists.mockResolvedValue(false);
      mockArgoClient.submitWorkflow.mockRejectedValue(
        new Error("Workflow submission failed"),
      );
  ...
  describe("getProvisioningStatus", () => {
    ...
    it("should return null for non-existent request", async () => {
      // Act
      const status = await service.getProvisioningStatus(
    ...
    it("should handle workflow status lookup failures", async () => {
      // Arrange
      const requestId = "req-123";
      mockArgoClient.getWorkflowStatus.mockRejectedValue(
  ...
  describe("generateWorkflowSpec", () => {
    it("should generate valid workflow spec for basic namespace", () => {
      ...
      const namespaceTemplate = spec.spec.templates.find(
        (t) => t.name === "create-namespace",
      );
      expect(namespaceTemplate).toBeDefined();
      expect(namespaceTemplate?.script?.source).toContain(
    ...
    it("should apply correct resource quotas for different tiers", () => {
      ...
      // Assert
      const rbacTemplate = spec.spec.templates.find(
        (t) => t.name === "setup-rbac",
      );
