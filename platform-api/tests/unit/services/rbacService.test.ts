...
describe("RBACService", () => {
  ...
  let mockClusterConfigService: jest.Mocked<
    ReturnType<typeof getClusterConfigService>
  >;
  let mockAzureADService: jest.Mocked<
    ReturnType<typeof getAzureADValidationService>
  >;
  ...
  beforeEach(() => {
    ...
    (getClusterConfigService as jest.Mock).mockReturnValue(
      mockClusterConfigService,
    );
    (getAzureADValidationService as jest.Mock).mockReturnValue(
      mockAzureADService,
    );
  ...
  describe("provisionNamespaceRBAC", () => {
    ...
    beforeEach(() => {
      mockClusterConfigService.getDefaultCluster.mockReturnValue(
        mockClusterConfig,
      );
    ...
    it("should successfully provision RBAC for a namespace", async () => {
      ...
      expect(mockAzureADService.validatePrincipalById).toHaveBeenCalledWith(
        testPrincipalId,
      );
      ...
      const appliedManifest =
        mockK8sClient.createCustomResource.mock.calls[0][0];
    ...
    it("should use specific cluster when provided", async () => {
      ...
      expect(mockClusterConfigService.getCluster).toHaveBeenCalledWith(
        "specific-cluster",
      );
    });
    ...
    it("should handle Kubernetes API errors", async () => {
      mockK8sClient.createCustomResource.mockRejectedValue(
