...
import { authenticateToken, requireRole, requireTeamAccess } from '../../../src/middleware/auth';
...
describe('Authentication Middleware', () => {
  ...
  describe('requireRole', () => {
    ...
    it('should allow access when user has any of the required roles', () => {
      ...
      const middleware = requireRole(['namespace:admin', 'team:frontend:developer']);
  ...
  describe('requireTeamAccess', () => {
    ...
    it('should allow access when user belongs to the requested team', () => {
      ...
      // Act
      requireTeamAccess(mockRequest as Request, mockResponse as Response, nextFunction);
    ...
    it('should deny access when user does not belong to requested team', () => {
      ...
      expect(mockResponse.status).toHaveBeenCalledWith(403);
      expect(mockResponse.json).toHaveBeenCalledWith({
        error: 'Access denied. You can only access resources for team: frontend'
      });
      expect(nextFunction).not.toHaveBeenCalled();
    });
