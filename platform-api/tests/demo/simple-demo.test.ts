...
import { mockNamespaces, mockProvisioningRequests, mockUsers } from '../fixtures/namespaces';
...
describe('🚀 Platform API Demo - Core Concepts', () => {
  ...
  describe('📋 1. Service Catalog Overview', () => {
    it('should showcase available service templates', () => {
      ...
      const microserviceTemplates = mockTemplates.filter(t => t.category === 'microservice');
      console.log(`🔍 Filtered to microservice templates: ${microserviceTemplates.length} found\n`);
    ...
    it('should demonstrate template configuration options', () => {
      const microserviceTemplate = mockTemplates.find(t => t.id === 'microservice-api');
      ...
      microserviceTemplate!.examples.forEach((example, index) => {
        ...
        console.log(`         Config: ${JSON.stringify(example.parameters, null, 8)}\n`);
      });
    });
  });
  ...
  describe('🏗️  2. Namespace Provisioning Showcase', () => {
    it('should demonstrate namespace provisioning requests', () => {
      ...
      console.log(`📊 Current Provisioning Requests (${mockProvisioningRequests.length} total):\n`);
      ...
      mockProvisioningRequests.forEach((request, index) => {
        ...
        if (request.workflowStatus) {
          console.log(`      Workflow: ${request.workflowStatus.phase} - ${request.workflowStatus.message || 'In progress'}`);
        }
  ...
  describe('👥 5. User & Team Management', () => {
    it('should showcase user roles and permissions', () => {
      ...
      // Team distribution
      const teamUsers = users.reduce((acc, user) => {
  ...
  describe('📊 6. Platform Analytics', () => {
    it('should showcase platform metrics and insights', () => {
      ...
      const avgCpuUsage = Math.round(mockNamespaces
        .filter(ns => ns.quota)
        .reduce((sum, ns) => sum + ns.quota!.cpu.percentage, 0) / mockNamespaces.length);
      
      const avgMemoryUsage = Math.round(mockNamespaces
        .filter(ns => ns.quota)
        .reduce((sum, ns) => sum + ns.quota!.memory.percentage, 0) / mockNamespaces.length);
      ...
      Object.entries(featureCounts)
        ...
        .forEach(([feature, count]) => {
          console.log(`   ${feature}: ${count} namespaces (${Math.round(count/totalNamespaces*100)}%)`);
        });
      console.log('');
      ...
      const successRate = Math.round((completedRequests / mockProvisioningRequests.length) * 100);

      const completedDeployments = mockDeployments.filter(d => d.status === 'completed').length;
      const deploymentSuccessRate = Math.round((completedDeployments / mockDeployments.length) * 100);
      ...
      console.log(`   Average Resource Utilization: ${(avgCpuUsage + avgMemoryUsage) / 2}%`);
      console.log('');
    });
  });
