...
describe('RBAC Integration Tests', () => {
  ...
  describe('RBAC Status and Management', () => {
        ...
        items: [
          {
            ...
            spec: {
              principalId: '12345678-1234-5678-9abc-123456789abc',
              roleDefinitionId: '/subscriptions/test/providers/Microsoft.Authorization/roleDefinitions/test-role-id',
              scope: '/subscriptions/test/resourceGroups/test-rg/providers/Microsoft.ContainerService/managedClusters/test-cluster/namespaces/test-namespace'
            },
            status: {
              phase: 'Ready'
            }
    ...
    test('should remove RBAC from namespace', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.message).toContain('RBAC removed from namespace test-namespace');
    });
    ...
    test('should return 404 for non-existent namespace RBAC', async () => {
      ...
      expect(response.body.message).toContain('RBAC configuration for namespace non-existent-namespace not found');
    });
  });
  ...
  describe('Error Handling and Resilience', () => {
    test('should handle Azure AD service failures gracefully', async () => {
      ...
      expect(response.body.message).toContain('Failed to validate Azure AD principal');
    });
  ...
  describe('Audit Trail Validation', () => {
    test('should log all RBAC operations for audit trail', async () => {
      ...
      const auditLogs = consoleSpy.mock.calls
        .flat()
        .filter(call => typeof call === 'string' && call.includes('RBAC_OPERATION'));
