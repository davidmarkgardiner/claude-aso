...
describe("RBAC Integration Tests", () => {
  ...
  describe("Enhanced RBAC Operations", () => {
    ...
    it("should validate managed identity authentication before RBAC operations", async () => {
      ...
      const mockValidateAuth = jest.spyOn(
        rbacService,
        "validateManagedIdentityAuthentication",
      );
  ...
  describe("ASO Template Instantiation", () => {
    it("should generate valid ASO RoleAssignment manifest", async () => {
      ...
      expect(manifest.apiVersion).toBe(
      ...
      expect(manifest.spec.principalId).toBe(
        "12345678-1234-1234-1234-123456789012",
      );
    ...
    it("should include proper labels and annotations", async () => {
      ...
      expect(manifest.metadata.labels["platform.io/namespace"]).toBe(
        "test-namespace",
      );
      expect(manifest.metadata.labels["platform.io/team"]).toBe("test-team");
      expect(manifest.metadata.labels["platform.io/environment"]).toBe(
        "production",
      );
      expect(manifest.metadata.labels["platform.io/component"]).toBe(
        "namespace-rbac",
      );
      expect(manifest.metadata.labels["platform.io/created-by"]).toBe(
        "platform-api",
      );

      expect(
        manifest.metadata.annotations["platform.io/created-at"],
      ).toBeDefined();
      expect(manifest.metadata.annotations["platform.io/role-definition"]).toBe(
        "aks-rbac-admin",
      );
    });
  });
  ...
  describe("Security Policy Integration", () => {
    it("should validate namespace creation against security policies", async () => {
      ...
      const result = await securityPolicyEngine.validateOperation(
        "create-namespace",
        context,
      );
    ...
    it("should reject reserved namespace names", async () => {
      ...
      expect(result.violations).toContain(
        "Cannot use reserved namespace names",
      );
  ...
  describe("Error Handling", () => {
    ...
    it("should validate principal ID format", async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.violations).toContain(
        "Invalid Azure AD principal ID format",
      );
    });
  });
});
