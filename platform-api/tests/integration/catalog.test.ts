...
describe("Catalog API Endpoints", () => {
  ...
  describe("GET /api/platform/catalog/templates", () => {
    ...
    it("should support filtering by category", async () => {
      ...
      if (templates.length > 0) {
        expect(templates.every((t) => t.category === "microservice")).toBe(
          true,
        );
      }
    });
    ...
    it("should support filtering by tags", async () => {
      ...
      if (templates.length > 0) {
        expect(
          templates.every(
            (t) => t.tags.includes("nodejs") || t.tags.includes("api"),
          ),
        ).toBe(true);
      }
    });
  ...
  describe("GET /api/platform/catalog/templates/:id", () => {
    ...
    it("should require authentication", async () => {
      // Act
      const response = await request(app).get(
        "/api/platform/catalog/templates/some-template",
      );
  ...
  describe("GET /api/platform/catalog/deployments/:id/status", () => {
    it("should return deployment status", async () => {
      ...
      const templateId =
        templatesResponse.body.data[0]?.id || "microservice-api";
    ...
    it("should require authentication", async () => {
      // Act
      const response = await request(app).get(
        "/api/platform/catalog/deployments/some-deployment/status",
      );
